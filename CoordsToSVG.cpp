...
int main() {
    ...
    // for (int j = 0; j < 100; j++) {
        ...
        bool done = false;
        for (int i = 1; i < coordss - 1; i++) {
            if (done) continue;
            if (newstroke_coords[usingg][i][0] != 0 && newstroke_coords[usingg][i][1] != 0) { SVG << "<circle cx='" << newstroke_coords[usingg][i][0] + offset << "' cy='" << newstroke_coords[usingg][i][1] + yoffset << "' r='1.26' />" << endl; }
            ...
            SVG << "<line x1='" << newstroke_coords[usingg][i][0] + offset << "' y1='" << newstroke_coords[usingg][i][1] + yoffset << "' x2='" << newstroke_coords[usingg][c][0] + offset << "' y2='" << newstroke_coords[usingg][c][1] + yoffset << "' stroke='black' stroke-width='2.52'/>" << endl;

        }
        SVG << "</svg>";
        SVG.close();
        eraseFileLine("svg/" + name + ".svg", "110");
        // string line;
        // ifstream iSVG("svg/" + name + ".svg");
        // ofstream oSVG("tmp");
        // while (getline(iSVG, line)) {
        //     // cout << line << endl;
        //     if (line.find("108") != string::npos) {
        //         line = "";
        //     }
        //     oSVG << line << endl;
        // }
